...
export function generateSlots(
    ...
    // ✅ Parse hours/minutes directly from the string — no Date object, no timezone shift
    const parseHHMM = (timeStr) => {
        // Handle Date objects, strings, or anything else
        let str;
        if (timeStr instanceof Date) {
            // Already a Date — extract UTC hours/minutes directly to avoid local offset
            return {
                hours: timeStr.getUTCHours(),
                minutes: timeStr.getUTCMinutes(),
            };
        }
        str = String(timeStr); // coerce to string just in case

        // Works for both "2026-04-17 10:00:00" and "10:00:00" formats
        const timePart = timeStr.includes("T")
            ? timeStr.split("T")[1]         // ISO format: "2026-04-17T10:00:00"
            : timeStr.split(" ")[1];        // Space format: "2026-04-17 10:00:00"
        const [hours, minutes] = timePart.split(":").map(Number);
        return { hours, minutes };
    };

    const { hours: startH, minutes: startM } = parseHHMM(availStartTime);
    const { hours: endH, minutes: endM } = parseHHMM(availEndTime);
